...
class SuperPoint {
 public:
  ...
  bool infer(const cv::Mat &image, Eigen::Matrix<double, 258, Eigen::Dynamic> &features, Eigen::Matrix<double, 1, Eigen::Dynamic> &features_score);
 ...
 private:
  ...
  bool process_output(const tensorrt_buffer::BufferManager &buffers, Eigen::Matrix<double, 258, Eigen::Dynamic> &features, Eigen::Matrix<double, 1, Eigen::Dynamic> &features_score);
  ...
  void find_high_score_index(std::vector<float> &scores, std::vector<std::vector<int>> &keypoints, int h, int w, double threshold);

  void sample_descriptors(std::vector<std::vector<int>> &keypoints, float *descriptors, std::vector<std::vector<double>> &dest_descriptors, int dim, int h, int w, int s = 8);
};
